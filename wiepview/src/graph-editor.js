...
export class GraphEditor {
  ...
  renderNode(node) {
    ...
    group.append(
      ...
      textElement(node.className, 18, 87, { class: "node-class" }),
      textElement(trimMiddle(node.id, 43), 18, 105, { class: "node-path" }),
    );
  ...
  renderPorts(group, node, style) {
    ...
    for (const direction of ["slv", "mst"]) {
      sides[direction].forEach((port, index) => {
        const point = localPortPoint(style, direction, index, sides[direction].length);
        group.append(
          svgElement("circle", {
          ...
          textElement(
            trimLabel(port.name, 18),
            point.x + (direction === "slv" ? 12 : -12),
            point.y + 3,
            {
              class: "port-label",
              "text-anchor": direction === "slv" ? "start" : "end",
            },
          ),
        );
      });
  ...
  portPoint(portId) {
    ...
    const local = localPortPoint(style, port.direction, index, sameSide.length);
...
function localPortPoint(style, direction, index, count) {
  ...
  return { x: direction === "slv" ? 0 : style.width, y };
}
